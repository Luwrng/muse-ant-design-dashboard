...
const updateRecordStatus = async (record, newStatus, setDataSource, fetchData, setActiveTab) => {
  try {
...
const approveColumns = ({ handleViewImage,
  openRejectModal,
  setDataSource,
  setActiveTab,
  onApproved }) => [
      ...
      render: (_, record) => (
        <Space>
          <FontAwesomeIcon
            icon={faCheck}
            onClick={async () => {
              await updateRecordStatus(record, "ACTIVE", setDataSource, onApproved, setActiveTab);
            }}
